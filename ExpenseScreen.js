...
const ExpenseScreen = ({ isDarkMode, expenses, onUpdateExpenses, navigation }) => {
    ...
    const loadExpenses = async () => {
      try {
        ...
        if (savedExpenses !== null) {
          onUpdateExpenses(JSON.parse(savedExpenses));
        }
      } catch (error) {
  ...
  const handleSaveExpense = (expenseData) => {
    if (editingIndex >= 0) {
      ...
      handleUpdateExpenses([...expenses, expenseData]);
    }
          ...
          <View style={styles.expenseBottomRow}>
            ...
            <View style={styles.expenseActions}>
              <TouchableOpacity 
                ...
                onPress={() => openEditExpenseModal(item, index)}
              >
...
const ExpenseModal = ({ 
  ...
  const handleSave = () => {
    ...
    const expenseData = {
      amount: amountValue,
      description: isEditing ? expense.description : description,
