...
import guantesImage from './guante.png';
  ...
  return (
    <View style={{ flex: 1, backgroundColor: isDarkMode ? '#121212' : '#fff' }}>
      <View style={tabStyles.tabContainer}>
        <TouchableOpacity 
          style={[tabStyles.tabButton, activeTab === 'ryu1' && tabStyles.activeTab]}
          onPress={() => setActiveTab('ryu1')}
        >
          <Text style={[tabStyles.tabText, activeTab === 'ryu1' && tabStyles.activeTabText]}>Ryu Sushi 1</Text>
        </TouchableOpacity>
        
        <TouchableOpacity 
          style={[tabStyles.tabButton, activeTab === 'ryu2' && tabStyles.activeTab]}
          onPress={() => setActiveTab('ryu2')}
        >
          <Text style={[tabStyles.tabText, activeTab === 'ryu2' && tabStyles.activeTabText]}>Ryu Sushi 2</Text>
        </TouchableOpacity>
        
        <TouchableOpacity 
          style={[tabStyles.tabButton, activeTab === 'ryu3' && tabStyles.activeTab]}
          onPress={() => setActiveTab('ryu3')}
        >
          <Text style={[tabStyles.tabText, activeTab === 'ryu3' && tabStyles.activeTabText]}>Ryu Sushi 3</Text>
        </TouchableOpacity>
      </View>
      
      {activeTab === 'ryu1' && <InventoryTab isDarkMode={isDarkMode} storageKey="ryuSushi1" tabStyles={tabStyles} />}
      {activeTab === 'ryu2' && <InventoryTab isDarkMode={isDarkMode} storageKey="ryuSushi2" tabStyles={tabStyles} />}
      {activeTab === 'ryu3' && <InventoryTab isDarkMode={isDarkMode} storageKey="ryuSushi3" tabStyles={tabStyles} />}
    </View>
  );
};
...
const InventoryTab = ({ isDarkMode, storageKey, tabStyles }) => {
    ...
    const loadData = async () => {
      try {
        ...
        if (savedData) {
          ...
          console.log('Se cargaron', parsed.length, 'elementos para', storageKey);
    ...
    try {
      if (selectedItem.exactQuantity !== undefined) {
        ...
        if (isPackageIndefinite) {
          ...
          const packageValue = packageInput === '' ? 0 : parseFloat(packageInput.replace(',', '.'));
          ...
          setItems(prevItems =>
            prevItems.map(item =>
              item.id === selectedItem.id
                ? { ...item, packages: packageValue, status: newStatus, packageIndefinite: false }
                : item
            )
          );
        }
    ...
    try {
      const resetItems = initialInventoryItems.map(item => {
        if (item.exactQuantity !== undefined) {
          ...
          return { ...item, packages: 0, status: 'agotado', packageIndefinite: false };
        }
      });
  ...
  return (
    <View style={[styles.container, themeStyles.container]}>
      <Text style={tabStyles.titleText}>INVENTARIO - {storageKey.toUpperCase()}</Text>
              ...
              <>
                ...
                {!isPackageIndefinite && packageInput ? (
                  <Text style={[styles.helpText, themeStyles.text]}>
                    {renderPackageText({packages: parseFloat(packageInput.replace(',', '.'))})}
                  </Text>
                ) : null}
              </>
            )}
